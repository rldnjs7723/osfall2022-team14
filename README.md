...
```
git clone -b proj1 https://github.com/rldnjs7723/osfall2022-team14.git
```
...
### 4.2
tizen kernel을 부팅한 이후 평균적으로 100개 정도의 프로세스가 생성되는 것을 확인했는데, 단순 시스템 실행에 필요한 프로세스도 상당히 많이 필요하다는 것을 배웠습니다.  
Tizen 운영체제는 IT기기에 주로 사용되는 것으로 알고 있는데, 현재 사용하는 Windows10 데스크탑 컴퓨터에서 실행되는 300개의 프로세스 중에서 Windows 프로세스가 100개 정도이므로 운영체제의 종류와 상관 없이 필수로 실행해야 하는 프로세스의 수는 비슷함을 알게 되었습니다.

### 4.3
리눅스를 사용할 때 virtualbox를 사용하는데, 얼마 전에(project 0 이후) 성능을 올려보려고 CPU 수를 권장수준 이상까지 올려 설정한 적이 있었습니다. 그것 때문에 이번 project에서 매 빌드마다 3시간씩 걸렸는데, 한동안 원인 파악을 하지 못하고, 다른 변경된 파일을 검토하고, MakeFile을 수정하려고 하거나 다른 커맨드를 조사하는 데에 많은 시간을 사용했습니다. 하지만, 과도한 CPU 할당이 문제였다는 것을 파악하고, 할당된 CPU 수를 다시 정상 수준으로 되돌리자 빌드 또한 정상적으로 수행되었습니다. 가상 머신을 사용할 때, CPU를 호스트 시스템보다 더 많이 할당하게 되면, 호스트의 성능이 저하되고 결과적으로, 역으로 가상머신의 저하시키는 경우가 생기게 되므로 주의해야 한다는 것을 배웠습니다.
