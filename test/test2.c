...
int main(int argc, char *argv[]) {
    ...
    printf("Running %d processes whose weights are all 10\n", process_num);
    
    for (int i = 1; i <= process_num; i++) {
        if (fork() == 0) {
            if (syscall(SCHED_SETWEIGHT, getpid(), 10) <= 0)
                exit(-1);
            ...
            printf("Process%d with weight %ld spent %fsec\n", i, syscall(SCHED_GETWEIGHT, getpid()), time_interval);
            exit(0);         
        }
    }
